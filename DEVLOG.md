...
- Really should declare each html element one time in `app.js` because declaring them every single time in each function is superfluous, could cut down on a lot of unnecessary declarations by just making them global values.
